...
export class AddUserComponent implements OnInit {
  ...
  public async createElement() {
    try {
      ...
      if(!this.samePassword || !this.sameEmail || !this.sameName || this.form.invalid) {
        this._general._spinner.hide();
        return;
      }
      const element: User = {
  ...
  public validateEmail(inputEmail: string): void {
    const validateEmail = this.users.some((user: User) => {
      return user.email.trim().toLowerCase() === inputEmail.trim().toLowerCase();
    });9
    if (validateEmail) {
  ...
  public validateName(): void {
    ...
    const validatefullName = this.users.some((user: User) => {
      return (
        `${user.name.toLowerCase().trim()}${user.last_name
          .toLowerCase()
          .trim()}` ===
